...
def calculo_imc():
    ...
    layout = [
        ...
        [sg.Text('Sexo: '), sg.OptionMenu(
            values=['Masculino', 'Feminino'], size=(8, 1), default_value='', key='_sexo')],
        [sg.Button('Entrar')]
    ]
...
#############################################################################################################
    if alt.isnumeric() and pes.isnumeric() and gen != '':
        ...
        if (gen.strip().upper()[0] == "M"):
            ...
            situacao = (
                "Algum parametro foi digitado incorretamente, não foi possivel definir sua Situação corretamente")
    else:
